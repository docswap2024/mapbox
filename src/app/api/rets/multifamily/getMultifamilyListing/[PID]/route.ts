...
export async function GET(req: NextRequest, props: { params: Promise<{ PID: string }> }) {
  ...
  console.log(`GET /api/rets/multifamily/getMultifamilyListing/${params.PID}`);
  ...
  try {
    const apiUrl = `${process.env.API_GATEWAY_URL_PROD}/retsMultifamily/getMultifamilyListing?PID=${pid}`;
