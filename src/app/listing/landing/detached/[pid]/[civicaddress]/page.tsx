...
const fetchListingData = async (pid: string) => {
  ...
  try {
    const res = await fetch(
      `${process.env.API_GATEWAY_URL_PROD}/retsDetached/getDetachedListing?PID=${PID}`,
...
export default async function ListingDetailPageWrapper({ params }: { params: Promise<{ pid: string; civicaddress: string }> }) {
  ...
  return (
    <>
      {listingData && listings && <ListingDetailPage getListing={listingData} listingType="detachedListing" listings={listings}/>}
    </>
  );
}
