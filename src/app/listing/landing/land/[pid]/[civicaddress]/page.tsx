...
const fetchListingData = async (pid: string) => {
  ...
  try {
    const res = await fetch(
      `${process.env.API_GATEWAY_URL_PROD}/retsLand/getLandListing?PID=${PID}`,
...
const fetchListings = async () => {
  let allLandListings = null;
  try {
    const response = await fetch(`${process.env.API_GATEWAY_URL_PROD}/retsLand/getAllLandListings`, 
      { cache: 'no-store' }
    );
    const data = await response.json();
    console.log(data);
    const filteredData = data.filter(item => item.Latitude && item.Longitude && item.Latitude.Value && item.Longitude.Value);
    allLandListings = filteredData;
    return allLandListings;
  } catch (error) {
    throw error;
  }
}
...
export default async function ListingDetailPageWrapper({ params }: { params: Promise<{ pid: string; civicaddress: string }> }) {  
  ...
  return (
    <>
      {listingData && listings && <ListingDetailPage getListing={listingData} listingType="landListing" listings={listings}/>}
    </>
  );
}
